...
export default function UpdateQuestion() {
  ...
  const getOneQuestion = () => {
    questionsBankAPI.getById(id).then((res) => {
      reset(res);
      for (const [key, value] of Object.entries(res)) {
        if (key === 'answers') {
          JSON.parse(value)?.forEach((element, index) => {
            setValue(`answer${element.id}`, element.answer);
            answers[index].answer = element.answer;
            answers[index].isCorrect = element.isCorrect;
          });
        }
      }
      setQuestion(res);
      ...
      {question ? (
        <form onSubmit={handleSubmit(onSubmit)} style={{ gap: '20px', display: 'flex', flexDirection: 'column' }}>
          ...
          {answers?.map((element, index) => (
            <Checkbox
              key={element.answer}
              defaultChecked={element.isCorrect ? true : false}
